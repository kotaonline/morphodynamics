...
if nargin < 7 || isempty(idx) || isempty(idy)
    idx=zeros(floor(sy/(N*(1-ol))),floor(sx/(M*(1-ol))));
    idy=zeros(floor(sy/(N*(1-ol))),floor(sx/(M*(1-ol))));
end
% The following two lines are original. I am changing ceil to floor
%xx=zeros(ceil((size(A,1)-N)/((1-overlap)*N))+1, ...
%    ceil((size(A,2)-M)/((1-overlap)*M)) +1);
xx=zeros(floor((size(A,1)-N)/((1-overlap)*N))+1, ...
    floor((size(A,2)-M)/((1-overlap)*M)) +1);
...
if nargin==8, 
    if ~isempty(maske)
        INP=zeros(size(maske(1).msk));
        for i=1:length(maske)
            INP=INP+double(maske(i).msk);
        end
    else 
        INP=zeros(size(A)); 
    end,
